...
# Configuration
TARGET_URL = "https://devfolio.co/search?happening=this_month&primary_filter=hackathons&type=application_open"
...
def clean_prize_details(prize_details):
    """Clean and filter prize details to remove duplicates and irrelevant information"""
    ...
    # Only keep entries that contain dollar amounts or meaningful prize information
    filtered_prizes = []
    for item in prize_details:
        ...
        # Skip navigation elements and UI components
        if any(word in item.lower() for word in ['home', 'blog', 'menu', 'filters', 'overview', 'navigate']):
            continue
            
        # Skip items that are likely to be UI elements
        if any(word in item.lower() for word in ['click', 'button', 'link', 'tab']):
            continue
        
        # Keep items with prize amounts or specific prize categories
        if re.search(r'(\$[\d,.]+|₹[\d,.]+|[\d,.]+\s*USD|[\d,.]+\s*INR)', item) or \
           any(word in item.lower() for word in ['prize', 'winner', 'reward', 'track', 'pool']):
            unique_prizes.add(item)
    
    # Convert back to list and sort by length (shorter first for readability)
    filtered_prizes = sorted(list(unique_prizes), key=len)
    
    # Limit to a reasonable number of items
    return filtered_prizes[:10]  # Limit to top 10 prize details

def clean_skills_required(skills):
    """Clean and filter skills to remove duplicates and ensure relevance"""
    if not skills:
        return []
    
    # Known technology skills to keep
    known_tech = [
        'python', 'javascript', 'typescript', 'react', 'node', 'java', 'c++', 'c#', 'php',
        'ai', 'ml', 'blockchain', 'web3', 'frontend', 'backend', 'fullstack', 'data science',
        'cloud', 'aws', 'azure', 'gcp', 'solidity', 'rust', 'golang', 'go'
    ]
    
    # Filter to known technologies and convert to lowercase
    filtered_skills = []
    for skill in skills:
        skill_lower = skill.lower()
        if any(tech in skill_lower for tech in known_tech):
...
def save_hackathons_to_csv(hackathons: List[Dict[str, Any]], filename: str) -> None:
    """
    Saves hackathons to a CSV file with cleaned data.
    """
    if not hackathons:
        print("No hackathons to save")
        return
    
    # Clean and optimize each hackathon entry
    for hackathon in hackathons:
        # Clean prize details
        if 'prizes_details' in hackathon and hackathon['prizes_details']:
            hackathon['prizes_details'] = clean_prize_details(hackathon['prizes_details'])
        
        # Clean skills required
        if 'skills_required' in hackathon and hackathon['skills_required']:
            hackathon['skills_required'] = clean_skills_required(hackathon['skills_required'])
...
async def crawl_hackathons():
    ...
    # URL for hackathons happening this month with open applications
    url = "https://devfolio.co/search?happening=this_month&primary_filter=hackathons&type=application_open"
    max_hackathons = 100
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Save results to CSV
            if all_hackathons:
                # Use pandas for additional data cleaning
                ...
                # Save using our custom function for better control over formatting
